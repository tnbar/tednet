...
Tensor contraction is the most typical operation for tensors, contracting two tensors into one tensor along the associated pairs of indices. As a result, the corresponding connected edges disappear while the dangling edges persist, as illustrated below.

.. image:: img/TC.png
  :width: 400

Tensor contractions among multiple tensors (e.g., tensor networks) can be computed by performing tensor contraction between two tensors many times. Hence, the order (or number of modes) of an entire tensor network is given by the number of dangling edges which are not contracted.
...
Tensor decomposition is a common technique for compressing Neural Networks, by decomposing a higher-order tensor into several lower-order tensors (usually matrices or order-3 tensors) that are sparsely interconnected through the tensor contraction operator. The basic tensor decomposition include CANDECOMP/PARAFAC (CP), Tucker, Block Term (BT), Tensor Train (TT) and so on.
...
The CP decomposition (also called CANDECOMP/PARAFAC decomposition)\cite{} factorizes a higher-order tensor into a sum of several rank-1 tensor components. The mathematical neural network layer format of utilizing CP decomposition is
...
Tucker decomposition \cite{} factorizes a higher-order tensor into a core tensor multiplied by a corresponding factor matrix along each mode. To be more specific,  the mathematical neural network layer format of utilizing tucker decomposition is
...
Matrix Tensor train (mTT) decomposition \cite{}(sometimes also called Tensor Train), also called Matrxi Product Operator(MPO) in quantum physics, factorizes a higher-order tensor into a linear multiplication of a series of order-4 core tensors.  The mathematical neural network layer format is
...
where :math:`\{R_0,R_1,\ldots,R_{N}\}` denote the TR-ranks, each node s an order-3 tensor and :math:`R_0=R_N`.  Compared with Tensor Train, it is not necessary for TR to follow a strict order when multiplying its nodes.
...
We can also represent the weight in other tensor formats. More details about tensorized convolution neural networks can be found here {Einconv}.
