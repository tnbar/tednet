...
class TTLSTM(_TNLSTM):
    def __init__(self, in_shape: Union[list, np.ndarray], hidden_shape: Union[list, np.ndarray],
                 ranks: Union[list, np.ndarray], drop_ih: float = 0.3, drop_hh: float = 0.35):
        ...
        ranks : Union[list, numpy.ndarray]
                1-D param :math:`\in \mathbb{R}^{m-1}`. The ranks of linear
        drop_ih : float
        ...
        hidden_shape[0] *= 4
        tn_block = TTLinear(in_shape, hidden_shape, ranks)
        super(TTLSTM, self).__init__(self.hidden_size, tn_block, drop_ih, drop_hh)
        self.reset_ih()
